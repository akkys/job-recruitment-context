...
const UpdateProfileDetails = (props) => {
  ...
  useEffect(() => {
    async function candidateProfileData() {
      const profileData = await Axios.get(
        "http://localhost:5000/candidate/" + props.match.params.id,
        {
          headers: { "x-auth-token": userData.token },
        }
      );
      setName(profileData.data.name);
      setMiniResume(profileData.data.miniResume);
      setRoles(profileData.data.roles);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await Axios.post(
        "http://localhost:5000/candidate/update/" + props.match.params.id,
        profileData,
        {
          headers: { "x-auth-token": userData.token },
        }
