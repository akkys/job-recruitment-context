...
const ScheduleInterview = (props) => {
  ...
  useEffect(() => {
    async function getInterviewData() {
      const interviewData = await axios.get(
        "http://localhost:5000/schedule/" + props.match.params.id,
        {
          headers: { "x-auth-token": userData.token },
        }
      );
      setName(interviewData.data.name);
      setDate(interviewData.data.date);
      setInterviewer(interviewData.data.interviewer);
      setInttype(interviewData.data.inttype);
      setEmail(interviewData.data.email);
    }
    getInterviewData();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(
        "http://localhost:5000/schedule/update/" + props.match.params.id,
        interviews,
        { headers: { "x-auth-token": userData.token } }
      );
      setInttype("");
            ...
            <div className="row">
              <div className="col-md-6">
                <div className="form-group">
                  ...
                  >
                    {interviewType.map((inttype, i) => {
                      return <option key={i}>{inttype}</option>;
                    })}
                  </select>
                </div>
              </div>
